/**
 * BellIcon — ícone de sino (notificações) da navbar.
 *
 * SVG do Figma (viewBox 0 0 16 16) com paths inline; usa `stroke` herdando a cor
 * via prop `color` (default `--muted-foreground`). Componente de domínio do
 * Design System Mutual — vive na lib, não nos apps.
 */

interface BellIconProps {
  className?: string
  color?: string
}

export function BellIcon({ className = 'w-4 h-4', color = 'var(--muted-foreground)' }: BellIconProps) {
  return (
    <div className={className}>
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="Icon">
          <path
            d="M6.8453 14C6.96233 14.2027 7.13064 14.371 7.33333 14.488C7.53601 14.605 7.76593 14.6666 7.99996 14.6666C8.234 14.6666 8.46392 14.605 8.6666 14.488C8.86929 14.371 9.0376 14.2027 9.15463 14"
            id="Vector"
            stroke={color}
            strokeLinecap="round"
            strokeLinejoin="round"
            strokeWidth="1.33333"
          />
          <path
            d="M2.17467 10.2173C2.08758 10.3128 2.0301 10.4315 2.00924 10.559C1.98837 10.6865 2.00501 10.8174 2.05714 10.9356C2.10926 11.0538 2.19462 11.1544 2.30284 11.225C2.41105 11.2956 2.53745 11.3332 2.66667 11.3333H13.3333C13.4625 11.3334 13.589 11.2959 13.6972 11.2254C13.8055 11.1549 13.891 11.0545 13.9433 10.9363C13.9955 10.8182 14.0123 10.6874 13.9916 10.5599C13.9709 10.4323 13.9136 10.3136 13.8267 10.218C12.94 9.304 12 8.33267 12 5.33333C12 4.27247 11.5786 3.25505 10.8284 2.50491C10.0783 1.75476 9.06087 1.33333 8 1.33333C6.93914 1.33333 5.92172 1.75476 5.17157 2.50491C4.42143 3.25505 4 4.27247 4 5.33333C4 8.33267 3.05933 9.304 2.17467 10.2173Z"
            id="Vector_2"
            stroke={color}
            strokeLinecap="round"
            strokeLinejoin="round"
            strokeWidth="1.33333"
          />
        </g>
      </svg>
    </div>
  )
}
